...
void resourceProvide()
{
    ...
    if(childProc==0){
        while(1){
            printf("Page size of the system is %d\n",getpagesize());
            printf("Please enter resource type,units to add for it (eg 2,4):\n");
            system("free");
            semkey = ftok("res.txt", 'c');
            ...
            else
            {
                ...
                int semctlcodeChild = semctl(semId,0,SETVAL);
                semctlcodeChild = semctl(semId,0,IPC_CREAT);
                struct sembuf sopChild [1];   //alternatively = {0,-1,0};
                sopChild[0].sem_num = 0;
                sopChild[0].sem_op = 1;  //seems to freeze when set at -1
                sopChild[0].sem_flg = 0;
                int semopcodeChild = semop(semId,sopChild,1);
                if(semopcodeChild<0){
                ...
                FILE *fpChild = fopen("res.txt", "r+");
                fd3 = fileno(fpChild);
                fd4 = fpChild->_fileno;

                int fstatReturnCode = fstat(fd3, &resFileStat);
                ...
                mmapPtrChild = mmap(NULL, resFileStat.st_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        ...
        while(1){
            ...
            for (size_t i = 0; i < strlen(buffer)/4; i++)
            {
                ...
                if(temp1==requestedResourceType){
                    if(requestedUnit+temp2>=10){
                        printf("Adding additional resources!\n");
                        sprintf(temp1Buffer, "%d", requestedResourceType);
                        sprintf(temp2Buffer, "%d", 9);
                        strcat(temp1Buffer, " ");
